...
where $\psi$ is a scaling parameter (controlling the height of the y-axis for the response, and is unconstrained), $\tau$ is an intercept, $s50$ is a parameter controlling the point at which the function reaches 50% of the maximum ($\psi$), and $s95$ is a parameter controlling the point at which the function reaches 95%. The parameter $s50$ is unconsrtrained, and $s95$ is constrained to be larger than $s50$.
...
We can then look at the important coefficients from the model. Here we're looking at 's50', 's95', and 's_max'.
...
For this model, the important parameters are the slope, 's_slope', and breakpoint, 's_cut'.
```{r output}
df = data.frame("par"=names(m$sd_report$value), "value"=as.numeric(m$sd_report$value),
  "sd"=as.numeric(m$sd_report$sd))
df
