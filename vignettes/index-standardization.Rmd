...
First we will create our SPDE mesh. We will use 150 knots for a balance between speed and accuracy in this vignette. You will likely want to use more for applied scenarios. You will want to make sure that increasing the number of knots does not change the conclusions.

```{r spde, fig.asp=0.8}
pcod_spde <- make_spde(pcod$X, pcod$Y, n_knots = 150)
plot_spde(pcod_spde)
```
