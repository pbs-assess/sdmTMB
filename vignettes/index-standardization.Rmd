...
```{r pred-sims}
pred_sims <- predict(m, newdata = qcs_grid, sims = 500)
dim(pred_sims)
```

`pred_sims` is just a matrix with width `sims` and a row for each row of data that was predicted on. You can process this matrix however you would like. For example, we can now calculate spatial uncertainty quickly:

```{r sims-cv-plot, cache=FALSE, fig.height=4, fig.width=4, out.width="3.5in", echo=TRUE}
