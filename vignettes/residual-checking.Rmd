...
vignette: >
  %\VignetteIndexEntry{residual-checking}
...
Start with some data simulated from scratch. We will simulate from a NB2 negative binomial observation model, a spatial random field, an intercept, and one predictor named 'a1' that will have a linear effect on the observed data.
...
Next, we will fit versions with various responses and predictors. The first model will use the Poisson instead of the NB2. The 2nd model will match the simulated data. The third model is missing the 'a1' predictor. We'll use a PC prior on the Matern parameters to aid in estimation.
...
Randomized quantile residuals at fixed effect MLE (Maximum Likelihood Estimates) and random effects that maximize the log likelihood at estimated fixed effects:
...
Here we will draw MCMC predictions and residuals. We will only take a single draw for speed:
...
The are obviously too few zeros in the data simulated from the Poisson model.
...
The above used simulations with the parameters fixed at there Maximum Likelihood Estimate (MLE) and predictions conditional on the fitted random effects. Alternatively, we could simulate with the parameters drawn from their joint precision matrix to encapsulate uncertainty about the parameters. This may be a better test for residual analysis, but this is an open area of research as far as I can tell.
