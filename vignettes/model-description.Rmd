...
* $\epsilon_{\s,t}$ are included if there are multiple time elements and `spatial_only = FALSE` (the default if there are multiple time elements)
...
$$
\begin{align}
  \mu_{\s,t} &= g^{-1} \left( \ldots + O_{\s,t} + \ldots \right),
\end{align}
$$
where $O_{\s,t}$ is an offset term---a **log transformed** variable without a coefficient. Offsets only make sense if the link is log.
...
Internally parameterized as the [robust version](https://kaskr.github.io/adcomp/group__R__style__distribution.html#gaecb5a18095a320b42e2d20c4b120f5f5).
