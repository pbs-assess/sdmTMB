...
symbols <- tibble::tribble(
  ...
  "$y$", "\\texttt{y\\_i}", "Observed response data",
  "$\\mu$", "\\texttt{mu\\_i}", "Mean",
  "$\\phi$", "\\texttt{phi}", "A dispersion parameter for a distribution",
  "$f$", "\\texttt{fit\\$family\\$link}", "Link function",
  "$f^{-1}$", "\\texttt{fit\\$family\\$linkinv}", "Inverse link function",
  "$\\boldsymbol{\\beta}$", "\\texttt{b\\_j}", "Parameter vector",
  "$\\boldsymbol{X}$", "\\texttt{X\\_ij}", "A predictor model matrix",
  "$O_{\\s, t}$", "\\texttt{offset}", "An offset variable at point $\\s$ and time $t$",

  "$\\omega_{\\s}$", "\\texttt{omega\\_s}", "Spatial random field at point $\\s$ (knot)",
  "$\\omega_{\\s}^*$", "\\texttt{omega\\_s\\_A}", "Spatial random field at point $\\s$ (interpolated)",
  "$\\zeta_{\\s}$", "\\texttt{zeta\\_s}", "Spatially varying coefficient random field at point $\\s$ (knot)",
  "$\\zeta_{\\s}^*$", "\\texttt{zeta\\_s\\_A}", "Spatially varying coefficient random field at point $\\s$ (interpolated)",
  "$\\epsilon_{\\s, t}$", "\\texttt{epsilon\\_st}", "Spatiotemporal random field at point $\\s$ and time $t$ (knot)",
  "$\\epsilon_{\\s, t}^*$", "\\texttt{epsilon\\_st\\_A}", "Spatiotemporal random field at point $\\s$ and time $t$ (interpolated)",
  "$\\delta_{\\s,t}$", "\\texttt{b\\_t}", "AR(1) or random walk spatiotemporal deviations (knot)",
  "$\\alpha_g$", "\\texttt{RE}", "IID random intercept deviation for group $g$",

  "$\\boldsymbol{\\Sigma}_\\omega$", "\\texttt{-}", "Spatial random field covariance matrix",
  "$\\boldsymbol{\\Sigma}_\\zeta$", "\\texttt{-}", "Spatially varying coefficient random field covariance matrix",
  "$\\boldsymbol{\\Sigma}_\\epsilon$", "\\texttt{-}", "Spatiotemporal random field covariance matrix",
  "$\\boldsymbol{Q}_\\omega$", "\\texttt{Q\\_s}", "Spatial random field precision matrix",
  "$\\boldsymbol{Q}_\\zeta$", "\\texttt{Q\\_s}", "Spatially varying coefficient random field precision matrix",
  "$\\boldsymbol{Q}_\\epsilon$", "\\texttt{Q\\_st}", "Spatiotemporal random field precision matrix",

  "$\\sigma_\\alpha^2$", "\\texttt{sigma\\_G}", "IID random intercept variance",
  "$\\sigma_\\epsilon^2$", "\\texttt{sigma\\_E}", "Spatiotemporal random field marginal variance",
  "$\\sigma_\\omega^2$", "\\texttt{sigma\\_O}", "Spatial random field marginal variance",
  "$\\sigma_\\zeta^2$", "\\texttt{sigma\\_Z}", "Spatially varying coefficient random field marginal variance",
  "$\\kappa_\\omega$", "\\texttt{kappa(0)}", "Spatial decorrelation rate",
  "$\\kappa_\\epsilon$", "\\texttt{kappa(1)}", "Spatiotemporal decorrelation rate",

  "$\\rho$", "\\texttt{ar1\\_rho}", "Correlation between random fields in subsequent time steps",
  "$\\boldsymbol{A}$", "\\texttt{A}", "Sparse projection matrix to interpolate between knot and data locations",
  "$\\boldsymbol{H}$", "\\texttt{H}", "2-parameter rotation matrix used to define anisotropy"
)
knitr::kable(symbols, caption = "Symbol notation, code representation (in model output or in model template code), and descriptions.", escape = FALSE, format = "latex", booktabs = TRUE,
   linesep = c(
    rep('', 7), # y, mean etc.
    '\\addlinespace',
    rep('', 7), # fields
...
\mu_{\bm{s},t} &=
f^{-1} \left( \bm{X}^{\mathrm{main}}_{\bm{s},t} \bm{\beta} +
O_{\s,t} +
\alpha_g +
...
* $\bm{X}^{\mathrm{main}}$, $\bm{X}^{\mathrm{tvc}}$, and $\bm{X}^{\mathrm{svc}}$ represent design matrices (the superscript identifiers 'main' = main effects, 'tvc' = time varying coefficients, and 'svc' = spatially varying coefficients);
...
* $\zeta_{\bm{s}}$ represents spatially varying coefficients (a random field), $\zeta_{\s} \sim \mathrm{MVN}(\bm{0},\bm{\Sigma}_\zeta)$;
...
$$
\begin{aligned}
\mu_{\bm{s},t} &= f^{-1} \left( \bm{X}^{\mathrm{main}}_{\bm{s},t} \bm{\beta} \ldots \right)
\end{aligned}
$$

Within `sdmTMB()`, $\bm{X}^{\mathrm{main}}_{\bm{s},t} \bm{\beta}$ is defined by the `formula` argument and represents the main-effect model matrix and a corresponding vector of coefficients. This main effect formula can contain optional penalized smoothers or non-linear functions as defined below.
...
$$
\begin{aligned}
\mu_{\bm{s},t} &= f^{-1} \left( \ldots + \omega_{\s} + \ldots \right),\\
...
$$
\begin{aligned}
  \mu_{\s,t} &= f^{-1} \left( \ldots +  \bm{X}^{\mathrm{tvc}}_{\s,t} \bm{\gamma_{t}} + \ldots \right),\\
...
where $\bm{\gamma_t}$ is an optional vector of time-varying regression parameters and $\bm{X}^{\mathrm{tvc}}_{\s,t}$ is the corresponding model matrix with covariate values.
...
$$
\begin{aligned}
  \mu_{\s,t} &= f^{-1} \left( \ldots + \bm{X}^{\mathrm{svc}}_{\s, t} \zeta_{\bm{s}} + \ldots \right),\\
...
Usually, $\bm{X}^{\mathrm{svc}}_{\s, t}$ would represent a prediction matrix that is constant spatially for a given time $t$ as defined by a one-sided formula supplied to `spatial_varying`.
