...
-   the Latin/Roman alphabet for data (except $\mathbf{Q}$ and $\mathbf{H}$, which are used by convention),

-   bold symbols for vectors or matrices (e.g., $\bomega$ is a vector and $\omega_{\s}$ is the value of $\bomega$ at point in space $\s$), with Latin symbols upright (e.g., $\mathbf{X}$, $\mathbf{Q}$),
...
-   subscripts and superscripts are placed outside bold commands (e.g., $\boldsymbol{\gamma}_{t}$ not $\boldsymbol{\gamma_t}$, and $\sigma_\alpha^2$ not $\sigma^2_\alpha$),
...
\mu_{\s,t} &=
f^{-1} \left( \bX^{\RM{main}}_{\s,t} \bbeta +
O_{\s,t} +
\bZ_{g} \mathbf{b}_{g} +
\bX^{\RM{tvc}}_{\s,t} \bgam_{t} +
\bX^{\RM{svc}}_{\s,t} \zeta_{\s} +
\omega_{\s} +
\epsilon_{\s,t} \right),
\end{aligned}
$$
...
-   $\omega_{\s}$ represents a spatial component (a random field), $\omega_{\s} \sim \RM{MVN}(\bs{0},\bSigma_\omega)$; and
-   $\epsilon_{\s,t}$ represents a spatiotemporal component (a random field), $\epsilon_{\s,t} \sim \RM{MVN}(\bs{0},\bSigma_{\epsilon})$.
...
$$
\begin{aligned}
  \gamma_{p,t=1} &\sim \RM{Uniform} \left(-\infty, \infty \right),\\
...
Code defined [within sdmTMB](https://github.com/pbs-assess/sdmTMB/blob/18a39eabc111e2179fa589f942c8820d87ad10df/src/utils.h#L505-L512) and implemented in the TMB likelihood at `src/sdmTMB.cpp` (e.g., lines 974–982).
...
Code defined [within sdmTMB](https://github.com/pbs-assess/sdmTMB/blob/18a39eabc111e2179fa589f942c8820d87ad10df/src/utils.h#L7-L43).
...
The list of supported families is included in the documentation on [additional families](https://pbs-assess.github.io/sdmTMB/reference/families.html), [delta models](https://pbs-assess.github.io/sdmTMB/articles/delta-models.html), and [Poisson-link delta models](https://pbs-assess.github.io/sdmTMB/articles/poisson-link.html). By default, the `delta_*` families don't use the Poisson link, but this structure can be specified with `delta_gamma(type = "poisson-link")`, which follows the formulation in @thorson2018poisson.

In the "standard" delta model implementation, sdmTMB constructs two internal models (formally, two "linear predictors"), with the first model representing presence-absence and the second model represting the positive component (such as catch rates in fisheries applications). 
