...
Below are tables of all major indices and symbols.

```{r indices}
subscripts <- structure(
  list(
    Symbol = c(
      "$\\s$", 
      "$t$",
...
knitr::kable(subscripts, caption = "Subscript notation", escape = FALSE)
```

```{r symbols, results='asis'}
symbols <- read.csv(
  system.file("symbols.csv", package = "sdmTMB"), 
  stringsAsFactors = FALSE
)
knitr::kable(symbols, 
  caption = 
    "Symbol notation, code representation (in model output or in model template code), and descriptions.", 
  escape = FALSE,
  linesep = c(
    rep('', 7), # y, mean etc.
    '\\addlinespace',
...
\mu_{\boldsymbol{s},t} &=
f^{-1} \left( \boldsymbol{X}_{1,\boldsymbol{s},t} \boldsymbol{\beta} +
O_{\s,t} +
\alpha_g +
\boldsymbol{X}_{2,\boldsymbol{s},t} \gamma_t +
\boldsymbol{X}_{3,t} \zeta_{\boldsymbol{s}} +
\omega_{\s} +
\epsilon_{\s,t} \right),
\end{aligned}
$$
...
* $\boldsymbol{X}_{1}$, $\boldsymbol{X}_{2}$, and $\boldsymbol{X}_{3}$ represent design matrices;
...
* $\zeta_{\boldsymbol{s}}$ represents spatially varying coefficients (a random field), $\zeta_{\s} \sim \mathrm{MVN}(\boldsymbol{0},\boldsymbol{\Sigma}_\zeta)$;
* $\omega_{\boldsymbol{s}}$ represents a spatial component (a random field), $\omega_{\s} \sim \mathrm{MVN}(\boldsymbol{0},\boldsymbol{\Sigma}_\omega)$; and
* $\epsilon_{\s,t}$ represents a spatiotemporal component (a random field), $\epsilon_{\s,t} \sim \mathrm{MVN}(\boldsymbol{0},\boldsymbol{\Sigma}_{\epsilon})$.

A single sdmTMB model will rarely, if ever, contain all of the above components. Next, we will split the model apart to describe the various parts in more detail using '$\ldots$' to represent the other optional components.

## Main effects

$$
\begin{aligned}
\mu_{\boldsymbol{s},t} &= f^{-1} \left( \boldsymbol{X}_{1,\boldsymbol{s},t} \boldsymbol{\beta} \ldots \right)
\end{aligned}
$$

Within `sdmTMB()`, $\boldsymbol{X}_{1,\boldsymbol{s},t} \boldsymbol{\beta}$ is defined by the `formula` argument and represents the main-effect model matrix and a corresponding vector of coefficients. This main effect formula can contain optional penalized smoothers or non-linear functions as defined below.

### Smoothers

Smoothers in sdmTMB are implemented with the same formula syntax familiar to mgcv users fitting GAMs (generalized additive models).
Smooths are implemented in the formula using `+ s(x)`, which implements a smooth from `mgcv::s()`.
Within these smooths, the same syntax commonly used in `mgcv::s()` can be applied, e.g. 2-dimensional smooths may be constructed with `+ s(x, y)`; smooths can be specific to various factor levels, `+ s(x, by = group)`; smooths can vary according to a continuous variable, `+ s(x, by = x2)`; the basis function dimensions may be specified, e.g. `+ s(x, k = 4)`; and various types of splines may be constructed such as cyclic splines to model seasonality, e.g. `+ s(month, bs = "cc", k = 12)`.
...
Spatial random fields, $\omega_{\s}$, are included if `spatial = 'on'` and omitted if `spatial = 'off'`.

$$
\begin{aligned}
\mu_{\boldsymbol{s},t} &= f^{-1} \left( \ldots + \omega_{\s} + \ldots \right),\\
\boldsymbol{\omega} &\sim \operatorname{MVNormal} \left( \boldsymbol{0}, \boldsymbol{\Sigma}_\omega \right),\\
\end{aligned}
$$
The marginal standard deviation of $\boldsymbol{\omega}$ is indicated by `Spatial SD` in the model output or as `sigma_O` in the output of `sdmTMB::tidy(fit, "ran_pars")`.
...
The marginal standard deviation of $\boldsymbol{\epsilon}_t$ is indicated by `Spatiotemporal SD` in the model output or as `sigma_E` in the output of `sdmTMB::tidy(fit, "ran_pars")`.
...
$$
\begin{aligned}
\mu_{\boldsymbol{s},t} &= f^{-1} \left( \ldots + \epsilon_{\s,t} + \ldots \right),\\
...
First-order auto regressive, AR(1), spatiotemporal random fields (`spatiotemporal = 'ar1'`) add a parameter defining the correlation between random field deviations from one time slice to the next.
They are defined as
...
$$
\begin{aligned}
  \mu_{\s,t} &= f^{-1} \left( \ldots + \gamma_{t} \boldsymbol{X}_{2,s,t} + \ldots \right),\\
...
$$
\begin{aligned}
  \mu_{\s,t} &= f^{-1} \left( \ldots + \boldsymbol{X}_{3,t} \zeta_{\boldsymbol{s}} + \ldots \right),\\
...
Usually, $\boldsymbol{X}_{3,t}$ would represent a predictor that is constant spatially for a given time $t$.
Currently, $\boldsymbol{X}_{3,t}$ is restricted to a single covariate (column vector), supplied by a one-sided formula to `spatial_varying`.
...
and $\kappa$ controls the spatial correlation range (decorrelation rate).
...
Internally, the parameter $\kappa$ and $\tau$ are converted to range and marginal standard deviation $\sigma$ as $\textrm{range} = \sqrt{8} / \kappa$ and $\sigma = 1 / \sqrt{4 \pi \exp \left(2 \log(\tau) + 2 \log(\kappa) \right) }$.

In the case of a spatiotemporal model with both spatial and spatiotemporal fields, if `share_range` is `TRUE` in `sdmTMB()` (the default), then a single $\kappa$ and range are estimated with separate $\sigma_\omega$ and $\sigma_\epsilon$. This often makes sense since the data are often only weakly informative about $\kappa$.
...
See `?sdmTMB::add_barrier_mesh` for examples.
...
Much of the guidance around diagnostics and glmmTMB also applies to sdmTMB, e.g. [the glmmTMB vignette on troubleshooting](https://cran.r-project.org/web/packages/glmmTMB/vignettes/troubleshooting.html).
