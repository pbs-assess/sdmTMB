...
These spatial random effects are assumed to be drawn from Gaussian Markov random fields (e.g. Cressie & Wikle, 2011; Latimer et al., 2009; Lindgren et al., 2011) with covariance matrices that are constrained by Matérn covariance functions (Cressie & Wikle, 2011).
...
pred_last$median <- apply(exp(p_last), 1, median)
pred_last$lwr <- apply(exp(p_last), 1, quantile, probs = 0.025)
pred_last$upr <- apply(exp(p_last), 1, quantile, probs = 0.975)
pred_last$cv <- round(apply(exp(p_last), 1, function(x) sd(x) / mean(x)), 2)
```

Plot the median estimated biomass densities for the last year.

```{r plot-median, fig.width = 6}
ggplot(pred_last, aes(X, Y, fill = median)) + geom_raster()+
  scale_fill_viridis_c(trans= "sqrt")
```
...
ggplot(pred_last, aes(X, Y, fill = cv)) + geom_raster()+
