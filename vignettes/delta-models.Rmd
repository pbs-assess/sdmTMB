...
sdmTMB does not have built-in hurdle models, but we can fit the two components separately and easily combine the predictions. 
In this case, our delta-gamma model involves fitting a binomial presence-absence model and then a model for the positive catches only with a Gamma observation distribution and a log link. 
...
A similar strategy can also be employed for zero-inflated count data, though additional zero-truncated distributions (yet to be implemented) may be required for some data sets. 
...
One can use different covariates in each model, but in this case we will just let the depth effect be more wiggly. 
...
We can easily inspect to effect of the smoothed covariate using the function `plot_smooth()`.
...
<!-- Here, we use fewer for speed, but more is always better. -->
...
ggplot(subset(pred, year == 2017), aes(X, Y, fill = cv)) + # 2011 as an example
...
The default for this function expects the simulations to still be in log space, so they we either need to `log(p_combined)`, or we can set `agg_function = function(x) sum(x)`. 
Note that the number of simulations influences these estimates and confidence intervals.
...
ggplot(ind, aes(year, est)) + geom_line() +
  ...
  ylab("Biomass (t)")
```
For more on modelling for the purposes of creating an index see the vignette on [Index standardization with sdmTMB](https://https://pbs-assess.github.io/sdmTMB/articles/index-standardization.html).
