...
These are models with one linear predictor for zero vs. non-zero data and another linear predictor for the positive component.
Hurdle models could also be implemented by fitting the two linear predictors as completely separate models and combining the predictions.
...
Here, we will show an example of fitting using the built-in delta functionality, as well as how to build each component model separately and then combine.
...
We can plot conditional effects of covariates (such as depth in the example model) for a delta model in the same way as described in the [intro vignette](https://pbs-assess.github.io/sdmTMB/articles/basic-intro.html), but with a couple additional considerations and options.
As before, we create a simplified data frame that we feed in to the predict function with `re_form = NA` for population-level predictions: 

```{r depth-eff}
nd <- data.frame(
  depth = seq(quantile(pcod$depth, 0.025), quantile(pcod$depth, 0.975), length.out = 100),
  year = c(2015L) # a chosen year
)

p <- predict(fit_dg, newdata = nd, se_fit = TRUE, re_form = NA)
str(p)
```

But this time our predictions contain three estimates. The `est` represents the combined prediction in log link space, while `est1` and `est2` represent estimates from each of the first and second linear predictors in their respective link spaces. 

```{r depth-eff-plot, fig.width = 4}
...
However, if you are wanting uncertainty on the effects of either of the component linear predictors, you will need to specific which linear predictor you want using `model = 1` or `model = 2`. 

```{r depth-eff-plot-model-1, fig.width = 4}
p1 <- predict(fit_dg, newdata = nd, se_fit = TRUE, re_form = NA, model = 1)
str(p1)

ggplot(p1, aes(depth, plogis(est),
  ...
  scale_x_continuous() +
  coord_cartesian(expand = F) +
  labs(x = "Depth (m)", y = "Probability present")
```

Note: Now the values returned for `est` and `est1` are identical and we must use the link associated with the component linear predictor that we selected.  

<!-- Currently, plotting effects of built-in delta models with `ggeffects` is not supported.  -->
<!-- We can also plot conditional effects of covariates using the package `visreg` by specifying each model component with `model=1` for the presence-absence model or `model=2` for the positive catch model. See the vignette on using [visreg](https://pbs-assess.github.io/sdmTMB/articles/visreg.html) with sdmTMB for more information. -->
...
# Delta models by fitting two models separately and combining predictions
