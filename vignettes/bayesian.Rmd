...
```{r mesh1}
pcod_mesh <- make_mesh(pcod, c("X", "Y"), cutoff = 20)
```
...
The `$tmb_obj` element can then be passed to `tmbstan`.
Here, 300 iterations and 2 chains is likely too small---but is used for the sake of speed (takes ~ 45 seconds).

```{r fitstan, message=FALSE, warning=FALSE}
options(mc.cores = parallel::detectCores()) # use rstan parallel processing
fit_stan <- tmbstan::tmbstan(fit$tmb_obj, iter = 300, chains = 2)
fit_stan
```

There are many options for plotting and diagnostics that be done with fitted model; all `stanfit` objects can be passed to bayesplot [@gabry_2019]:

```{r plotstan}
plot(fit_stan) # must `library(rstan)` first
bayesplot::mcmc_areas(fit_stan, regex_pars = "^b")
bayesplot::mcmc_trace(fit_stan, regex_pars = "^b")
```

# Fixing parameters at their MLE

Sometimes some of the spatial correlation parameters can be challenging to estimate.
...
Here we will demonstrate this using a simulated dataset.
...
sim_dat <- sdmTMB_simulate(
  ...
  sigma_O = 0.2,
  seed = 42,
  B = c(1, -0.4) # B0 = intercept, B1 = a1 slope
)
```
...
Fit a spatial random field GLMM:
...
fit <- sdmTMB(
  ...
  spatial = "on",
  priors = sdmTMBpriors(
    b = normal(location = c(0, 0), scale = c(5, 1))
  )
)
fit
```
...
Now we can pass our model to tmbstan.
Again, we are only using 1000 iterations and 2 chains so this vignette builds quickly.
...
fit_stan <- tmbstan::tmbstan(
  fit_mle$tmb_obj,
  iter = 1000, chains = 2,
  seed = 9278 # ensures repeatability
)
fit_stan
```

The `Rhat` values look reasonable (< 1.05).
The `n_eff` (number of effective samples) looks reasonable for inference about the mean for all parameters except the intercept (`b_j[1]`).
...
bayesplot::mcmc_pairs(fit_stan, pars = pars_plot, off_diag_fun = "hex")
```
...
`simulate()` will take draws from the joint parameter posterior and add observation error.
...
ggplot(sim_dat, aes(X, Y, colour = post_mean, size = abs(post_mean))) +
  geom_point() +
  scale_color_viridis_c()

ggplot(sim_dat, aes(X, Y, colour = post_sd, size = post_sd)) +
  geom_point() +
  scale_color_viridis_c()
```
...
In the above examples we did not use any priors.
...
Then we can use this model as above.
Optionally we could use `update()` to fix parameters before passing the `$tmb_obj` element to `tmbstan::tmbstan()`.
