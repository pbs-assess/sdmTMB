...
The function uses parallelization by default, but this can be turned off with the `parallel` argument.
...
<!-- The `sdmTMB_cv()` function is intended to fit a model multiple times using the same fold IDs and data. -->
<!-- In an application with a single split, e.g. 10% of the data being used as a test set, this may be overkill---but this approach may be worthwhile to avoid coding mistakes with likelihood calculations. For example, -->
