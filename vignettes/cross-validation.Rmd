...
The goals of some sdmTMB applications may be focused on spatial prediction; these include making prediction to new spatial regions (e.g. unsampled areas or areas not sampled in every year). For these types of models we recommend exploring folds using the `blockCV` or `spatialsample` packages. In general, these spatial sampling approaches assign observations that are spatially autocorrelated to the same fold. Accounting for the spatial correlation can lead to better estimates of covariate effects, as well as prediction errors.
...
Cross validation in sdmTMB is implemented using the `sdmTMB_cv` function, with the `k_folds` argument specifying the number of folds (defaults to 8). The function uses parallelization by default, but this can be turned off with the `parallel` argument.      
...
The `sdmTMB_cv` function is intended to fit a model multiple times using the same fold IDs and data. In an application with a single split, e.g. 10% of the data being used as a test set, this may be overkill -- but this approach may be worthwhile to avoid coding mistakes with likelihood calculations. For example, 
...
Note: in the above model, we're not using year as a factor because it 
...
Silge, J. 2021. spatialsample: Spatial Resampling Infrastructure, R package version 0.1.0. url = {https://CRAN.R-project.org/package=spatialsample

Valavi R., J. Elith, J. Lahoz-Monfort, and G. Guillera-Arroita. 2019. blockCV: An R
package for generating spatially or environmentally separated folds
for k-fold cross-validation of species distribution models. Methods in 
Ecology and Evolution, 10:225 -- 232. https://doi.org/10.1111/2041-210X.13107
  
Yao, Y., A. Vehtari, D. Simpson, and A. Gelman. 2018. Using stacking to average Bayesian predictive distributions. Bayesian Analysis, 13:917 -- 1007. https://doi.org/10.1214/17-BA1091
