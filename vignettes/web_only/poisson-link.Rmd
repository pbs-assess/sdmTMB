...
## What is the connection to the complementary log-log link ('cloglog')?

From [Wikipedia](https://en.wikipedia.org/wiki/Generalized_linear_model): "The cloglog model corresponds to applications where we observe either zero events (e.g., defects) or one or more, where the number of events is assumed to follow the Poisson distribution.[6]..."

The cloglog link function is defined as

$$
\log(-\log(1 - x)),
$$

which has the following inverse link function

$$
1 - \exp(-\exp(x)).
$$

You can see how that inverse link is the same as part of our Poisson-link delta model.
The linear first linear predictor is on $\log n$, so

$$
p = 1 - \exp(-\exp(\log(n))).
$$

This is the inverse cloglog link function.
*However*, the 'trick' here is that group density $n$ affects both encounter probability $p$ and positive catch rates $r$:
...
The offset is added to the linear predictor for $\log n$, i.e., `exp(offset) * n`. For example if the offset is defined as log area swept, $a$, then

$$
p = 1 - \exp(-a \cdot n).
$$

Therefore, the encounter probability goes to 1 as area swept (effort) and underlying group numbers density increase towards infinity.

## Why call it a Poisson-link?

I can only guess. The cloglog is derived from the theory of a Poisson and although the cloglog formulation appears here, the interaction of the two model components makes it different from a pure cloglog and so I assume it needed another name.
