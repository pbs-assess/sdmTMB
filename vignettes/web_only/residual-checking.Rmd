...
pkgs <- dplyr_installed && ggplot_installed && glmmTMB_installed && DHARMa_installed && sdmTMBextra_installed && tmbstan_installed
...
For delta models, or models of presence-absence data, additional diagnostics can be generated. A first commonly used diagnostic is the AUC (Area Under the Curve), quantifying the ability of a model to discriminate between the two classes; this is done from the Receiver Operating Characteristic (ROC) curve, which plots the true positive rate versus false positive rate. There are a number of packages to calculate AUC in R, but this can be done with the `pROC` package, where inputs are a vector of 0s and 1s (or factor equivalents) in the raw data, and a vector of estimated probabilities (generated from a call to `predict()`, as shown below). The `plogis()` function is needed to convert estimated values in logit space to probabilities in normal space. 

```{r roc, eval=FALSE}
pred <- predict(sdmtmb_model) # presence-absence model
roc <- pROC::roc( actual_values, plogis(pred$est) )
auc <- pROC::auc(roc)
```

With a delta model, two estimated values are returned, so only the first would be used, 
```{r, eval=FALSE}
roc <- pROC::roc(actual_values, plogis(pred$est1))
```

AUC may be sensitive to imbalances in the data, however, and alternative metrics may better approximate skill. To highlight an example of using true skill score (implemented in packages like `SDMtune`), we can create a new binary response and fit a model to that.
...
fit_bin <- sdmTMB(observed ~ 1, data = dat, family = binomial(), mesh = mesh)
```

Next, we can generate predicted probabilities and classes using a threshold of 0.5

```{r}
pred <- predict(fit_bin)
pred$p <- plogis(pred$est)
pred$pred_01 <- ifelse(pred$p < 0.5, 0, 1)
```

Next we create a confusion matrix and calculate the true skill skore,

```{r}
conmat <- table(pred$pred_01, pred$observed)
