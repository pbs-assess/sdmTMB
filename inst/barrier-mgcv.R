...
library(ggplot2)
ggplot(data.frame(x = v, y = w, z = y), aes(x, y, colour = z)) + geom_point() +
...
bnd <- INLA::inla.nonconvex.hull(loc_xy, convex = .5)
...
spde <- sp2
plot(spde)
m <- sdmTMB(z ~ 1 + b, data = dat, spde = spde, silent = FALSE)
...
mb <- sdmTMB(z ~ 1 + b, data = dat, spde = bspde, silent = FALSE, barrier = TRUE,
...
g1 <- ggplot(pred, aes(x, y, fill = omega_s)) + geom_raster() +
  geom_point(data = dat, aes(x, y, size = z), inherit.aes = FALSE, pch = 21) +
  scale_fill_gradient2(high = cols[1], low = cols[3], mid = cols[2]) + theme(legend.position = "none") +
  geom_path(data = as.data.frame(xym), aes(V1, V2), inherit.aes = FALSE)

g2 <- ggplot(pb, aes(x, y, fill = omega_s)) +
  geom_raster() +
  geom_point(data = dat, aes(x, y, size = z), inherit.aes = FALSE, pch = 21) +
  scale_fill_gradient2(high = cols[1], low = cols[3], mid = cols[2]) + theme(legend.position = "none") +
  geom_path(data = as.data.frame(xym), aes(V1, V2), inherit.aes = FALSE)
