...
Type objective_function<Type>::operator()()
{
  ...
  for (int m = 0; m < n_m; m++) {
    if (threshold_func == 1) { // linear hockey stick
      ...
      s95(m) = exp(b_threshold(1,m)); // delta between s95 and s50
...
<<<<<<< HEAD
      s95(m) = exp(b_threshold(1,m)); // not actually s95; log 'delta' here for MI model
    ...
    if (threshold_func > 0) {
      ...
      for (int m = 0; m < n_m; m++) {
        ...
        if (threshold_func == 1) { // linear
          for (int m = 0; m < n_m; m++) {
            for (int i = 0; i < n_p; i++) {
              proj_fe(i,m) += sdmTMB::linear_threshold(proj_X_thresh(i,m), s_slope(m), s_cut(m));
            }
          }
        } else { // logistic
          for (int m = 0; m < n_m; m++) {
            for (int i = 0; i < n_p; i++) {
              proj_fe(i,m) += sdmTMB::logistic_threshold(proj_X_thresh(i,m), s50(m), s95(m), s_max(m), mi_est);
            }
          }
        }
      }
