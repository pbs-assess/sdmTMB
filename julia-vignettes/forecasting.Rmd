...
pkgs <- dplyr_installed && ggplot_installed && sdmTMB_installed && spatstat_installed
...
Then, we will fit a model of cod density that includes depth variables. The argument `extra_time` in the `sdmTMB` function is how we will add in forecasting.
...
To include spatio-temporal variation as an AR(1) process, set, `spatiotemporal="AR1"`
...
We can also include both a random walk intercept and an AR(1) process by removing the intercept by adding `0` to the model equation and including the argument `time_varying = ~1`
...
* Spatiotemporal field uncertainty grows without data, for both AR(1) and Random Walk processes
