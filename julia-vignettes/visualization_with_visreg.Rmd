...
For a model with multiple continuous variables, for instance, this example with simulated data
```{r}
N = 300
...
We can similarly use `visreg` to visualize how the two variable interact to effect the response
```{r}
visreg2d(fit3, xvar = "hour_effect", yvar = "log_depth")
```
