...
See the [sdmTMBextra](https://github.com/pbs-assess/sdmTMBextra) package
for the function `predict_mle_mcmc()`, which can generate the MCMC
samples to pass to the `mcmc_samples` argument. Ideally MCMC is run
until convergence and then the last iteration can be used for residuals.
