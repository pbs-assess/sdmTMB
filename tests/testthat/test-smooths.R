...
test_that("A model with 2 s() splines works", {
  ...
  pcod_spde <- make_mesh(d, c("X", "Y"), cutoff = 25)
  m3 <- sdmTMB(data = d,
    # formula = log(density) ~ s(depth_scaled), #+ s(year, k = 4),
    # formula = log(density) ~ s(year, k = 4),
    formula = log(density) ~ s(depth_scaled) + s(year, k = 5),
    spde = pcod_spde,
    control = sdmTMBcontrol(map_rf = TRUE), do_fit = T)
  ...
  m$tmb_data$b_smooth_end

  m$sd_report
  exp(as.list(m$sd_report, "Estimate")$ln_smooth_sigma)
  as.list(m$sd_report, "Estimate")$b_smooth
  as.list(m$sd_report, "Std. Error")$b_smooth
  ...
  m_brms <- brms::brm(log(density) ~ s(depth_scaled) + s(year, k = 5), data = d,
    chains = 4, control = list(adapt_delta = 0.98), cores = 4, iter = 1000)
