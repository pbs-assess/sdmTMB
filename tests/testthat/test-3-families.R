...
if (suppressWarnings(require("INLA", quietly = TRUE))) {
  ...
  test_that("Gamma mixture fits", {
    ...
    m <- sdmTMB(data = d, formula = y ~ 1,
                family = gamma_mix(),
                spatial = "off",
                control = sdmTMBcontrol(newton_loops = 1))
    expect_equal(plogis(m$model$par[["logit_p_mix"]]), 0.1, tolerance = 0.1)
  ...
  test_that("Lognormal mixture fits", {
    skip_on_ci()
    skip_on_cran()
    d <- pcod[pcod$year == 2017 & pcod$density > 0, ]
    spde <- make_mesh(d, c("X", "Y"), cutoff = 10)
  ...
  test_that("Delta Gamma mixture fits", {
    skip_on_ci()
    skip_on_cran()
    d <- pcod
    spde <- make_mesh(d, c("X", "Y"), cutoff = 10)
    m <- sdmTMB(data = d, formula = density ~ 1,
                mesh = spde, family = delta_gamma_mix(),
                spatial = "off",
                control = sdmTMBcontrol(newton_loops = 1))
    expect_true(all(!is.na(summary(m$sd_report)[,"Std. Error"])))
    expect_length(residuals(m), nrow(d))
  ...
  test_that("Delta lognormal mixture fits", {
    skip_on_ci()
    skip_on_cran()
    d <- pcod
    spde <- make_mesh(d, c("X", "Y"), cutoff = 10)
    m <- sdmTMB(data = d, formula = density ~ 1,
                mesh = spde, family = delta_lognormal_mix(),
                spatial = "off",
                control = sdmTMBcontrol(newton_loops = 1))
    #expect_true(all(!is.na(summary(m$sd_report)[,"Std. Error"])))
    #expect_length(residuals(m), nrow(d))
